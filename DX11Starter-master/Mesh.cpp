...
{
	BufferCreation(vertexArray, vertexNum, indices, indiceNum, device, context);
}

Mesh::Mesh(const char* file, Microsoft::WRL::ComPtr<ID3D11Device> device, Microsoft::WRL::ComPtr<ID3D11DeviceContext> context)
{
...
// File input object
	...
	obj.close();
	BufferCreation(&verts[0], vertCounter, &indices[0], indexCounter, device, context);
...
void Mesh::BufferCreation(Vertex* vertexArray, unsigned long long vertexNum, unsigned int* indices, unsigned long long indiceNum, Microsoft::WRL::ComPtr<ID3D11Device> device, Microsoft::WRL::ComPtr<ID3D11DeviceContext> context)
{
